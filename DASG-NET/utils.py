...
def create_input_files(dataset, karpathy_json_path, image_folder, captions_per_image, min_word_freq, output_folder,
    ...
    assert dataset in {'coco', 'flickr8k', 'flickr30k','RSICD','Sydney','UCM'}
    ...
    for img in data['images']:
        if imgnum is not None:
            count += 1
            if count > imgnum:  # FIXME: fewer images
                break
        captions = []
    ...
    with open("./data/test_image_paths.json", 'w', encoding='utf-8') as json_file:  # FIXME:  Store the test img paths
        json.dump(test_image_paths, json_file, ensure_ascii=False)
...
def create_input_files222(dataset, karpathy_json_path, image_folder, captions_per_image, min_word_freq, output_folder,
                       max_len=100):
    """
    Creates input files for training, validation, and test data.
...
def save_checkpoint(checkpoint_name, epoch, epochs_since_improvement, encoder, decoder, encoder_optimizer, decoder_optimizer,
                    metrics, is_best, final_args, output_dir='./', current_name=None, best_name=None, infos=None, histories=None):
    """
    ...
    filename = current_name or ('checkpoint_' + checkpoint_name + '.pth.tar')
    filepath = os.path.join(output_dir, filename)  # 最终参数模型
    torch.save(state, filepath)

    if decoder_optimizer is not None:
        torch.save(decoder_optimizer.state_dict(), os.path.join(output_dir, "optimizer.pth"))
    if encoder_optimizer is not None:
        torch.save(encoder_optimizer.state_dict(), os.path.join(output_dir, "encoder_optimizer.pth"))

    if infos is not None:
        with open(os.path.join(output_dir, "infos.pkl"), "wb") as f:
            pickle.dump(infos, f, protocol=pickle.HIGHEST_PROTOCOL)
    if histories is not None:
        with open(os.path.join(output_dir, "histories.pkl"), "wb") as f:
            pickle.dump(histories, f, protocol=pickle.HIGHEST_PROTOCOL)

    # If this checkpoint is the best so far, store a copy so it doesn't get overwritten by a worse checkpoint
    if is_best:
        best_filename = best_name or ('BEST_' + filename)
        torch.save(state, os.path.join(output_dir, best_filename))
        if decoder_optimizer is not None:
            torch.save(decoder_optimizer.state_dict(), os.path.join(output_dir, "best_optimizer.pth"))
        if encoder_optimizer is not None:
            torch.save(encoder_optimizer.state_dict(), os.path.join(output_dir, "best_encoder_optimizer.pth"))
    #torch.save(state, os.path.join('./models_checkpoint3/', 'Last_epoch_'+ filename))
    #torch.save(state, os.path.join('./models_checkpoint3/', 'epoch_'+str(epoch) + filename))
...
"""
    ...
    for scorer, method_i in scorers:
        try:
            if method_i == "SPICE":
                gts = {i: ref[i] for i in range(len(ref))}
                res = {i: hypo[i] for i in range(len(hypo))}
                score_i, scores_i = scorer.compute_score(gts, res)
            else:
                score_i, scores_i = scorer.compute_score(ref, hypo)
        except BrokenPipeError:
            print("Warning: metric {} skipped due to BrokenPipeError (likely METEOR subprocess).".format(method_i))
            continue
        except Exception as e:
            print("Warning: metric {} failed with error: {}".format(method_i, e))
            continue
