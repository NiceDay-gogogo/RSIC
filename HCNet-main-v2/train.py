...
if __name__ == '__main__':
    ...
    # Data parameters
    parser.add_argument('--data_folder', default="./data/NWPU_images1",help='folder with data files saved by create_input_files.py.')
    ...
    # NOTE: encoder_fusion(hfam/hfamv2) 相关模型已清理，本脚本仅保留 baseline 与 ablation 所需模式
    parser.add_argument('--decoder_mode', default="lstm_attention",
                        help='decoder_mode: lstm_attention | aoa | aoa_hfam_cfim')
    ...
    parser.add_argument('--encoder_lr', type=float, default=5e-4, help='learning rate for encoder if fine-tuning.')
    ...
    for encoder_layers, decoder_layers in [(3,3)]: #,,(0,6),(2,2),
        ...
        def build_models():
            if args.decoder_mode == "aoa_hfam_cfim":
        ...
        train_loader = torch.utils.data.DataLoader(
            CaptionDataset(args.data_folder, args.data_name, 'TRAIN', transform=transforms.Compose([transforms.RandomHorizontalFlip(),normalize])),
            batch_size=args.batch_size, shuffle=True, num_workers=args.workers, pin_memory=True)
        val_loader = torch.utils.data.DataLoader(
            CaptionDataset(args.data_folder, args.data_name, 'VAL', transform=transforms.Compose([transforms.RandomHorizontalFlip(),normalize])),
            batch_size=args.batch_size, shuffle=True, num_workers=args.workers, pin_memory=True)
