...
"""Generate and save 20 AoAAllAttrProb heatmap montages on Sydney.

This is the Sydney counterpart to the UCM "save N montages" workflow:
- Pick 20 candidate images from a split (default: test) with basic scene diversity.
- Run `visualize_attention_heatmap_batch.py` once to generate outputs under vis/<image_id>/.
- Copy ALL 20 `montage.png` into figures/ for convenient browsing / paper inclusion.
- Write a manifest recording scene, image_id, caption and scores.

Notes:
- We keep the same scoring fields as the pick4 script (caption_f1/focus/visual_prob) so
  users can later choose representative samples manually or programmatically.
- This script does NOT pick a subset; it always copies all N.
"""
...
def main() -> None:
    ...
    ap.add_argument("--fig_out_dir", type=str, default="figures/sydney_attr_heatmaps_20")
    ...
    candidates = choose_candidates(images, gt_caps, scene_rules, num=int(args.num))
    if len(candidates) < int(args.num):
    ...
    for idx, (scene, image_id) in enumerate(candidates, start=1):
        ...
        out_png = os.path.join(
            args.fig_out_dir,
            f"{idx:02d}_{scene}_id{int(image_id)}_score{s:.3f}.png",
        )
        shutil.copyfile(montage_path, out_png)
    ...
    print("All 20 montages copied to:", args.fig_out_dir)
