...
"""Generate 20 AoAAllAttrProb heatmap montages, then pick the best 4.

What it does:
1) Pick 20 candidate test images covering different scenes (keyword heuristic on GT captions).
2) For each candidate, run `visualize_attention_heatmap.py` (AoAAllAttrProb) and save results under `vis/.../<image_id>/`.
3) Read `token_metrics.json` to score "how representative" the heatmaps are.
4) Pick 4 best images with distinct scenes when possible and copy their `montage.png` into `figures/...`.

Scoring heuristic (simple but effective):
- Prefer sharper attention (higher focus) on non-stopword tokens.
- Prefer higher visual_prob mass (P[next token is attribute word]).
...
def choose_candidates(
    ...
    # Prefer non-"other" scenes first; take at most ~2 per scene when possible.
    ...
    # Pass 1..K: one per scene each pass
...
def score_from_metrics(metrics: dict, gt_caps: List[str]) -> Tuple[float, float, float, float]:
    ...
    # Weighted sum: prioritize caption correctness, then attention concentration.
...
def main() -> None:
    ...
    ap.add_argument("--vis_out_dir", type=str, default="vis/ucm_attr_heatmaps_20")
    ap.add_argument("--out_dir", type=str, default="figures/ucm_attr_heatmaps_best4")
    ...
    # Run visualization once for all candidates (loads model+DataLoader only once)
    ...
    # Sort by score and pick distinct scenes
    ...
    # If still short, fill with remaining best
    if len(picked) < int(args.num_pick):
        for row in all_sorted:
            if row in picked:
                continue
