...
"""Generate Fig10-style minimal qualitative figures for RSICD.

Output: figures/fig10_rsicd_XX_<scene>_id<image_id>.png (+ .spec.json)

Selection policy (default):
- Prefer samples where our model improves over baseline (delta score)
- Keep scene diversity using COCO file_name prefix (e.g., airport_348.jpg -> airport)
"""
...
def pick_scene_from_captions(captions: List[str], scene_rules: List[Tuple[str, List[str]]]) -> str:
...
def main():
    ...
    ap.add_argument(
        "--our",
        default="eval_results/.cache_aoa_rsicd_test_test.json",
        help="Our predictions json",
    )
    ap.add_argument(
        "--baseline",
        default="eval_results/.cache_show_tell_rsicd_rl_best_test_test.json",
        help="Baseline predictions json",
    )
    ap.add_argument(
        "--gt",
        default="coco-caption/annotations/captions_rsicd_test.json",
        help="COCO GT captions json",
    )
    ap.add_argument("--min_delta", type=float, default=0.2, help="Minimum (our-baseline) score delta")
    ap.add_argument("--min_our", type=float, default=0.5, help="Minimum our score")
    args = ap.parse_args()
    ...
    scene_rules: List[Tuple[str, List[str]]] = [
        ("airport", ["airport", "airfield", "runway", "runways", "aircraft", "plane", "planes", "terminal"]),
        ("beach", ["beach", "coast", "coastal", "shore", "sea", "ocean", "waves"]),
        ("bridge", ["bridge", "bridges"]),
        ("center", ["center", "centre", "downtown"]),
        ("church", ["church", "cathedral"]),
        ("commercial", ["commercial", "shops", "shop", "mall", "market"]),
        ("denseresidential", ["dense residential", "dense houses", "many houses", "many buildings"]),
        ("mediumresidential", ["residential", "houses", "house", "buildings", "building"]),
        ("sparseresidential", ["sparse", "few houses", "few buildings"]),
        ("desert", ["desert", "sand", "sandy"]),
        ("farmland", ["farmland", "farms", "farm", "fields", "field", "crops", "crop"]),
        ("forest", ["forest", "woods", "wood", "trees", "tree"]),
        ("industrial", ["industrial", "factory", "warehouses", "warehouse", "chimney"]),
        ("meadow", ["meadow", "grass", "lawn", "lawns"]),
        ("mountain", ["mountain", "mountains", "hill", "hills"]),
        ("park", ["park", "garden"]),
        ("parking", ["parking", "parked cars", "cars parked", "cars"]),
        ("playground", ["playground", "court", "courts"]),
        ("pond", ["pond", "lake", "lakes"]),
        ("port", ["port", "harbor", "harbour", "dock", "docks", "ship", "ships", "boats"]),
        ("railwaystation", ["railway", "railroad", "station", "train", "trains"]),
        ("resort", ["resort", "hotel"]),
        ("river", ["river", "canal", "waterway"]),
        ("school", ["school", "campus"]),
        ("square", ["square", "plaza"]),
        ("stadium", ["stadium", "football field", "soccer field", "field is", "court is"]),
        ("storagetanks", ["storage tank", "storage tanks", "tanks"]),
        ("viaduct", ["viaduct", "overpass", "overpasses", "interchange"]),
    ]

    # Candidate must have: GT, filename, and both predictions.
    candidates: List[int] = []
    for image_id in gt_caps.keys():
    ...
    # Score all candidates and keep best per-scene by delta.
    per_scene: Dict[str, List[Tuple[float, float, int]]] = {}
    scored_all: List[Tuple[float, float, str, int]] = []

    for image_id in candidates:
        file_name = id2file.get(image_id, "")
        scene = scene_from_filename(file_name) if file_name else "other"

        # Some RSICD COCO files use pure numeric names (e.g., 00624.jpg).
        # In that case, infer a more readable scene label from GT captions.
        if scene.isdigit():
            scene = pick_scene_from_captions(gt_caps.get(image_id, [])[:gt_count], scene_rules)
        ...
        s_our = score_prediction(our_preds.get(image_id, ""), gt_vocab, syn_inv)
        s_base = score_prediction(base_preds.get(image_id, ""), gt_vocab, syn_inv)
        delta = s_our - s_base

        scored_all.append((delta, s_our, scene, image_id))
        per_scene.setdefault(scene, []).append((delta, s_our, image_id))

    for scene, arr in per_scene.items():
        arr.sort(key=lambda x: (x[0], x[1]), reverse=True)

    picked: List[Tuple[str, int]] = []
    used_ids = set()

    # 1) Scene-diverse picks: best delta per scene.
    for scene in sorted(per_scene.keys()):
        for delta, s_our, image_id in per_scene[scene]:
            if image_id in used_ids:
                continue
            if s_our < args.min_our or delta < args.min_delta:
                continue
    ...
    # 2) Fill remainder by global delta.
    if len(picked) < args.num:
        scored_all.sort(key=lambda x: (x[0], x[1]), reverse=True)
        for delta, s_our, scene, image_id in scored_all:
            if image_id in used_ids:
                continue
            if s_our < args.min_our or delta < args.min_delta:
                continue
    ...
    # 3) If still not enough (too strict thresholds), relax delta, then score.
    if len(picked) < args.num:
        for delta, s_our, scene, image_id in scored_all:
            if image_id in used_ids:
                continue
            if s_our < args.min_our:
                continue
            picked.append((scene, image_id))
            used_ids.add(image_id)
            if len(picked) >= args.num:
    ...
    for idx, (scene, image_id) in enumerate(picked[: args.num], start=1):
        ...
        out_png = os.path.join(args.outdir, f"fig10_rsicd_{idx:02d}_{scene}_id{image_id}.png")
        out_spec = os.path.join(args.outdir, f"fig10_rsicd_{idx:02d}_{scene}_id{image_id}.spec.json")
