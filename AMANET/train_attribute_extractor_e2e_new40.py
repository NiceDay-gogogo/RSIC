"""
端到端训练属性特征提取器 (new40版本)

使用 ResNet101 作为 backbone，直接从原始图像训练属性分类器。

使用方法:
    python train_attribute_extractor_e2e_new40.py \
        --input_json data/rsicd_with_attributes_new40.json \
        --images_root data/RSICD_images \
        --save_path save/attribute_extractor_e2e_new40.pth
"""
...
def main():
    parser = argparse.ArgumentParser(description="End-to-end train attribute extractor (new40) with ResNet backbone on RSICD")
    ...
    parser.add_argument("--save_path", type=str, default="save/attribute_extractor_e2e_new40.pth")
    ...
    if args.log_path is None:
        if save_dir:
            args.log_path = os.path.join(save_dir, "train_attribute_extractor_e2e_new40.log")
        else:
            args.log_path = "train_attribute_extractor_e2e_new40.log"
    ...
    for epoch in range(1, args.num_epochs + 1):
        train_loss = train_one_epoch(model, train_loader, optimizer, device, label_smoothing=args.label_smoothing)
        ...
        if map_macro > best_map:
            ...
            if args.patience > 0 and patience_counter >= args.patience:
                log(f"Early stopping triggered after {epoch} epochs (no improvement for {args.patience} epochs)")
                break

        scheduler.step()

    log(f"Training finished. Best mAP_macro: {best_map:.4f}")
    log_f.close()
